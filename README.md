...
[Дизайн проекта.](https://www.figma.com/file/RIkypcTQN5d37g7RRTFid0/Algososh_external_link?node-id=0%3A1) Там можно найти необходимые цвета, размеры и примерную раскадровку визуализации.

В качестве примера можно подглядеть работу подобных сервисов по визуализации, например, [visualgo.net](https://visualgo.net/en), или полностью самостоятельно разобраться в задаче. Дерзайте!

# Демонстрация проекта на GitHub Pages
https://kristinayandex.github.io/algososh-master/
...
Перед тем как приступить к визуализации, на страницу надо добавлены основные компоненты:
...
Два кандидатов на сортировку подсвечиваются цветом `#D252E1`. Уже отсортированные элементы выделяются `#7FE051`. 
...
Добавлена  кнопка лоадер, чтобы избежать прерывания запущенной анимации.

Анимация должна выполняться плавно. Для установки интервала можно использовать значение 1000 мс.

## Последовательность Фибоначчи

На этом экране сгенерируется `n` чисел последовательности Фибоначчи. 
**Компоненты**

Перед тем как приступить к визуализации, на страницу добавлены основные компоненты:
...
Например, вы ввели 4, тогда на экране должен появиться ряд 1, 1, 2, 3, 5. Ограничен ввод, в инпут можно вводить только положительные целые числа. И так как последовательность бесконечна, добавлена максимальная граница ввода 1 ≤ `n`≤ 19.

**Визуализация**

Элементы должны отображаться постепенно. Сначала появляется один, потом второй, третий и так до `n`. Пока ряд появился не полностью, отображается лоадер на кнопку, чтобы избежать повторных кликов.
...
На этом экране визуализирован алгоритм сортировки выбором и пузырьком
...
Чтобы задать массив случайных чисел, необходимо написана функция `randomArr`. При написании учтено, что:

- массив должен состоять из целых чисел $[0; 100]$,
- минимальное количество элементов массива `minLen = 3`, максимальное `maxLen = 17`.

Теперь сгенерированный массив отображается на странице. Максимальное значение элемента массива равно 100, это же значение является процентным соотношением высоты столбца. В качестве максимальной высоты принято 340px.
...
Для анимации сортировки добавлены два цвета:

- `#D252E1` — элементы, которые сортируем;
- `#7FE051` — отсортированные элементы массива.

Приведём пример сортировки выбором по возрастанию для массива `[2, 34, 17, 100, 50]`. Чтобы отсортировать элементы выбором, сначала найден минимальный элемент, поставлен первым элементом в массив и так далее:
...
Добавляется лоадер на активную кнопку и дизейблятся все остальные контролы, пока идёт анимация сортировки.

Сортировка массива выполняется плавно. Для установки интервала можно использовать значение 1000 мс.
...
Перед тем как приступить к визуализации работы со стеком, на страницу надо добавлены основные компоненты:
...
Ограничения: например, при нажатии на кнопку отображается на ней лоадер, а остальные контролы дизейблятся. Ограничен ввод значения до четырёх символов, иначе значение будет странно смотреться в компоненте Circle.

Все анимации на странице выполняются плавно. Для установки интервала можно использовать значение 500 мс.

---

## Очередь

На этом экране вам визуализировано удаление и добавление элементов в структуру данных «очередь».

**Компоненты**

Перед тем как приступить к визуализации работы с очередью, на страницу добавлены основные компоненты:
...
Если ввести в инпут значение 2 и нажать «Добавить», элемент отображается под индексом 0. Также добавляются на элемент указатели `head` и `tail`. Инпут при этом очищается.

![Очередь с одним элементом](README_static/Untitled%208.png)

Очередь с одним элементом

При добавлении элементов в очередь позиция tail смещается, на долю секунды выделяется новый элемент цветом `#D252E1`.
...
Теперь если нажать «Удалить», из очереди извлекается элемент под индексом 0 (на долю секунды подсвечивается элемент `#D252E1`), a `head` перемещён на элемент с индексом 1.
...
Ограничения: например, при нажатии на кнопку отображается на ней лоадер, а остальные контролы дизейблятся. Ограничен ввод значения до четырёх символов, иначе значение будет странно смотреться в компоненте Сircle.

---

## Связный список

На этом экране необходимо реализовано удаление и добавление элементов в связный список. 

**Компоненты**

Перед тем как приступить к визуализации работы с очередью, на страницу надо добавлены основные компоненты.
...
После успешного добавления 10 стоит под порядковым номером 2 и указывает на 34. Новый добавленный элемент выделяется цветом. Через долю секунды убираются все цветовые выделения и лоадер на кнопке — вставка завершена.

![Добавление по индексу. Новый элемент в списке](README_static/Untitled%2015.png)

Добавление по индексу. Новый элемент в списке

**При удалении элемента по индексу** сначала выделяются цветом элементы, пока не достигнем нужного индекса. Затем очищаем значение в элементе и снизу отображаем маленький кружок с удаляемым значением.
...
Обратите внимание, что при добавлении новый элемент отображается над элементами списка, а при удалении — под ними.

Все анимации на странице выполняются плавно. Для установки интервала можно использовать значение 500 мс.

Ограничения: Добавляется лоадер на активную кнопку и дизейблятся все остальные контролы. Ограничивается ввод значения до четырёх символов, иначе значение будет странно смотреться в компоненте Circle.
